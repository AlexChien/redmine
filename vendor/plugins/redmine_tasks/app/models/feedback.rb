class Feedback
  ...
  def self.final_image
    Attachment.in_final(1).in_output(0).all(:order=>"created_on ASC").each do |a|
      ...
      if container.class == Issue
        FileUtils.copy("#{Attachment.storage_path}/#{a.disk_filename}","#{OUTPUT}/#{container.code}#{container.style_effect}.jpg")
  ...
  def self.final_file
    to = Date.today
    file = File.new("#{OUTPUT}/0310-SKTRSP-#{to.to_s.gsub('-','')}","w")
  ...
  def self.final_layout
    to = Date.today
    file = File.new("#{OUTPUT}/0310-SKTRSPLAYOUT-#{to.to_s.gsub('-','')}","w")
  ...
  def self.final_error
    ...
    file_name = "#{OUTPUT}/0310-TASKFAILED-#{to.to_s.gsub('-','')}.xls"
  ...
  def self.final_execption
    ...
    file_name = "#{OUTPUT}/0310-TASKEXCEPTION-#{to.to_s.gsub('-','')}.xls"
    book.write(file_name)
  end
  ...
  def self.final_status_change
    to = Date.today
    file = File.new("#{OUTPUT}/0310-TASKSTATUS-#{to.to_s.gsub('-','')}","w")
